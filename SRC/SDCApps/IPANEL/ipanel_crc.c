...
// CRC-ITU  CRC32 查找表
static const unsigned int crctab256[] = 
{
...
unsigned int ipanel_GetCrc32(const unsigned char* pData, int nLength)
{
	...
	for (i=0; i<nLength; i++)
	{
		crc = (crc << 8) ^ crctab256[((crc >> 24) ^ (*pData)) & 0xff];
        pData++;
	}
...
/******测试,发送和接收用同一个数组模拟********/
static int ipanel_crc16_test()
{
    unsigned char p[]= {0xa0,0xb0,0xff, 0xff};
    unsigned short crc;      

	crc = ipanel_GetCrc16(p,2);

    p[2]=crc&0x00ff;   //将计算的CRC码加到信息序列后面
    p[3]=crc>>8&0x00ff; //封装成代CRC码的发送数组缓冲
    ipanel_porting_dprintf("p[2]=%x,p3=%x\n",p[2],p[3]);

#if 1
	crc = ipanel_GetCrc16(p,4); //对信息码＋CRC码共同计算得出CRC=0xf0b8，假设接收端就是这些内容
	ipanel_porting_dprintf("crc is %x\n",crc);

	if( 0xf0b8 == crc)
		ipanel_porting_dprintf("check ok!\n");
	else 
		ipanel_porting_dprintf("check fail!\n");
#else
	if( ipanel_IsCrc16Good(p,4))
		ipanel_porting_dprintf("check ok!\n");
	else 
		ipanel_porting_dprintf("check fail!\n");
#endif
