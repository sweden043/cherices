...
/****************************************************************************/
/*
 * Filename:      llserhostmodem.c
 *
 * Description:   Low level serial driver for host connected serial modem
 *
 * Author:        bradforw
 *
 ****************************************************************************/
/* $Header: LLSERHOSTMODEM.C, 4, 11/4/03 2:52:50 PM, Angela Swartz$
 ****************************************************************************/
...
/* locals */
static volatile UART_16550* serport = (UART_16550*)MODEM_HOST_MEMBASE;
...
static int llserhostmodem_isr( u_int32 id, bool bFIQ, PFNISR* pfnChain);
static int config( unsigned int rate, unsigned int bits,
      unsigned int stops, bool parity, bool even );
static void enable( bool ena );

/*
 * PLLSERINTERFACE llseropen_HostModem( PSER_CB_FNS pcbfns )
 *
 * parameters:
 ...
 */
PLLSERINTERFACE llseropen_HostModem( PLLSERCALLBACKS pcb )
{
   bool crit;
   ...
   if( pcb && !isrInstalled )
   {
      if (MODEM_HOST_INTERRUPT != GPIO_INVALID)
      {
         eRetCode = cnxt_gpio_set_int_edge(MODEM_HOST_INTERRUPT, POS_EDGE);
         ...
         eRetCode = cnxt_gpio_int_register_isr
               (MODEM_HOST_INTERRUPT,
               (PFNISR)llserhostmodem_isr,
               FALSE,
               FALSE,
               &chained_isr);
         if (eRetCode != CNXT_GPIO_STATUS_OK)
   ...
   if( pcb )
   {
      ...
      eRetCode = cnxt_gpio_int_enable(MODEM_HOST_INTERRUPT);
      if (eRetCode != CNXT_GPIO_STATUS_OK)
      {
         opened = FALSE;
         return (0);
...
static int serclose( void  )
{
   ...
   if( isrInstalled )
   {
      eRetCode = cnxt_gpio_int_disable(MODEM_HOST_INTERRUPT);
      if (eRetCode != CNXT_GPIO_STATUS_OK)
      {
         return (0);
      }
                        ...
                        unsigned int stops, bool parity, bool even )
{
   ...
   serport->iir = 0x01;    /* set the LSB first */
...
static int llserhostmodem_isr( u_int32 id, bool bFIQ, PFNISR* pfnChain)
{
   ...
   /* make sure we have vectors hooked */
   if ( cbfn == NULL )
   {
      *pfnChain = chained_isr;
      return ( RC_ISR_NOTHANDLED );
      ...
      switch( iir & 0x07 )
      {
         ...
         case 0 :    /* modem status */
            ...
            if( msr & 0x02 )  /* delta dsr */
            {
               ...
               {
                  cbfn->flowReadyHandler( msr & 0x20 );
               }
            }
            if( msr & 0x04 )
            {
 ...
 * Modifications:
 * $Log: 
 *  4    mpeg      1.3         11/4/03 2:52:50 PM     Angela Swartz   CR(s): 
 *        7777 7778 deleted the code of modem reset in open function, now modem
 *         reset is performed in SERMODEM open function
 *  3    mpeg      1.2         10/28/03 1:14:59 PM    Bobby Bradford  CR(s): 
 *        7731 7732 
 *        1) Removed setDTR function (redundant with setFlowReady() function
 *        
 *  2    mpeg      1.1         10/15/03 4:57:17 PM    Angela Swartz   CR(s): 
 *        7637 7636 added setDTR function to set DTR on or off, will be used by
 *         cnxt_modem_ioctl()
 *  1    mpeg      1.0         9/22/03 4:58:52 PM     Bobby Bradford  
 * $
 * 
 ****************************************************************************/
