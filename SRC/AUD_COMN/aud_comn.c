...
static void audio_set_volume_internal(int percentage, AUD_ATTN_MODULE module)
{
   ...
   Shift = Shift + 4;   /* 4 bit attenuation to avoid bad sound. */
   if (Shift >= 15)
   {
      Shift = 15;
   }
...
/*********************************************************************  
    FUNCTION:    cnxt_check_av_channel_status                                    
                                                                      
    PARAMETERS:  channel_id
                 1: video 
                 2: audio
                                                                      
    DESCRIPTION: to check if audio or video channel has data input at the moment  
                    
    RETURNS:     TRUE - Audio or video channel has data input
                        FALSE- Audio or video channel has no data input
                        
    CONTEXT:     This function cannot be called in interrupt 
         
 *********************************************************************/
bool cnxt_check_av_channel_status(int channel_id)
{
   LPMPG_ADDRESS AudWritePtr;
   LPMPG_ADDRESS VidWritePtr;
   u_int32 level1,level2;
   int ret = FALSE;
    
   VidWritePtr = (LPMPG_ADDRESS)DPS_VIDEO_WRITE_PTR_EX(gDemuxInstance);
   AudWritePtr = (LPMPG_ADDRESS)DPS_AUDIO_WRITE_PTR_EX(gDemuxInstance);
   
   switch(channel_id)
    {
      case 1:/*video*/
        level1 = *((u_int32*)VidWritePtr);
        //task_time_sleep(20);
        task_time_sleep(10);
        level2 = *((u_int32*)VidWritePtr);
        if(level1 == level2)
            ret = FALSE;
        else
            ret = TRUE;
        /*trace("Videowptr: level1 = 0x%x,level2= 0x%x\n", level1,level2);*/
        break;
      case 2:/*audio*/
        level1 = *((u_int32*)AudWritePtr);
        //task_time_sleep(20);
        task_time_sleep(10);
        level2 = *((u_int32*)AudWritePtr);
        if(level1 == level2)
            ret = FALSE;
        else
            ret = TRUE;
         /*trace("Audiowptr: level1 = 0x%x,level2= 0x%x\n", level1,level2);*/
        break;
      default:
        break;
    }
...
void ipanel_get_apts(u_int32 *pts_value)
{
    ...
    aPtsH = (u_int32)(*glpAPTSHi);
    aPtsL = (u_int32)(*glpAPTSLo);

    *pts_value = (aPtsH<<31) | (aPtsL >> 1);
}
