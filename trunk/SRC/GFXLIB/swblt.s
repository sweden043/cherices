...
;* CN49XX Project                                                       *
;*                                                                      *
;* Copyright (C) Conexant Systems Inc 1998-2002.   All rights reserved. *
;*                                                                      *
;************************************************************************
;----------------------------------------------------------------------
;$Header: swblt.s, 5, 7/30/03 3:14:50 PM, Larry Wang$
;----------------------------------------------------------------------
;$Log: 
; 5    mpeg      1.4         7/30/03 3:14:50 PM     Larry Wang      SCR(s) 7076
;        :
;       Change *.MAC and *.INC to *.a
;       
; 4    mpeg      1.3         12/18/02 1:10:56 PM    Dave Wilson     SCR(s) 5185
;        :
;       Removed some redundant emulation code.
;       
; 3    mpeg      1.2         5/20/02 5:44:02 PM     Bobby Bradford  SCR(s) 3834
;        :
;       Made AREA directive conditional to remove INTERWORK warning
;       
; 2    mpeg      1.1         7/3/01 10:41:54 AM     Tim White       SCR(s) 2178
;        2179 2180 :
;       Merge branched Hondo specific code back into the mainstream source 
;       database.
;       
;       
; 1    mpeg      1.0         3/6/00 1:29:56 PM      Lucy C Allevato 
;$
;  
;     Rev 1.4   30 Jul 2003 14:14:50   wangl2
;  SCR(s) 7076 :
;  Change *.MAC and *.INC to *.a
;  
;     Rev 1.3   18 Dec 2002 13:10:56   dawilson
;  SCR(s) 5185 :
;  Removed some redundant emulation code.
;  
;     Rev 1.2   20 May 2002 16:44:02   bradforw
;  SCR(s) 3834 :
;  Made AREA directive conditional to remove INTERWORK warning
;  
;     Rev 1.1   03 Jul 2001 09:41:54   whiteth
;  SCR(s) 2178 2179 2180 :
;  Merge branched Hondo specific code back into the mainstream source database.
;  
;  
;     Rev 1.0.1.0   09 May 2001 13:31:40   prattac
;  DCS914 Removed support for Neches.
;  
;     Rev 1.0   06 Mar 2000 13:29:56   eching
;  Initial revision.
;  
;------------------------------------------------------------------------
...
NEVER   EQU 0
        ...
        AREA GraphicsData, DATA, READWRITE
...
CacheConfig     DCD     0
    ...
    ELSE
        AREA Graphics, CODE, READONLY, INTERWORK
    ENDIF
USE_GCP EQU 0

        EXPORT SetupFB
        EXPORT HWCopy2Screen
        EXPORT HWSolidFill
        EXPORT SWCopy2Screen
        EXPORT SWCopy2Memory
        EXPORT SWSolidFill
        EXPORT SWXORFill
        EXPORT RepColor
        EXPORT FillBytes
...
GcpStart
        mENTER 0
        ; Add any special code needed for emulation here
        mLEAVE
;-----------------------------------------------------------------------------;
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; SWSolidFill(BYTE *pDest, int DeltaX, int ExtX, int ExtY, int Color, int X, int Y, int bpp)
;
;Description: Fill a rectangular region on the screen with a single color. No
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; SWXORFill(BYTE *pDest, int DeltaX, int ExtX, int ExtY, int Color, int X, int Y, int bpp)
;
;Description: Fill a rectangular region on the screen with a single color. No
;             clipping is performed but it is XORed with dest
;Input:
;       r0      BYTE *pDest: Address of the start of the FB
;       r1      int DeltaX : Pitch
;       r2      int ExtX   : width in bytes of the rectangle
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; SWCopyLineXBPP
;               Copies a single line of 8/16 bpp pixels from the source to the destination.
; Entry
;       r0 Dst byte address
;       r1 Src byte Address (always byte aligned)
;       r2 DstX
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; SWCopy2Screen
;               Copies a rectangle from memory to the screen without clipping or overlapping
; Entry
...
LineCopy2ScrFuncTable DCD 0,0,SWCopyLine4Bpp,SWCopyLineXBpp,SWCopyLineXBpp,SWCopyLineXBpp
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; SWCopy2Memory
;               Copies a rectangle from the screen to memory without clipping or overlapping
; SWCopy2Memory(int pDest, int pSrc, int DstPitch, int SrcPitch, int ExtX, int ExtY, int SrcX, int SrcY, int Bpp);
; Entry
; r0 dest pixel address
; r1 src pixel address
; r2 DestPitch
...
LineCopy2MemFuncTable DCD 0,0,SWCopyLine4Bpp,SWCopyLineXBpp,SWCopyLineXBpp,SWCopyLineXBpp
...
NextLine2Screen
        ...
        mPOP    "{r0,r1,r2,r3,r4,r5,r6,r7,r8,r9,r10,lr}"
        bx      lr
...
LineLoop12Screen
        ...
        mPOP    "{r0,r1,r2,r3,r4,r5,r7,r8,r9,lr}"
        bx      lr

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; HWSolidFill(BYTE *pDest, int DeltaX, int ExtX, int ExtY, int Color, int X, int Y, int bpp)
;
;Description: Fill a rectangular region on the screen with a single color. No
;             clipping is performed.
;Input:
...
FillLineLoop1
        ...
        mPOP    "{r0,r1,r2,r3,r4,lr}"
        bx      lr


;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; FUNCTION: Copies a single line with the source and destination that are aligned
; Input:
;       r0 = Destination
;       r1 = Source
;       r2 = ExtX in bytes
...
LineDoneBothAligned
        ...
        ENDIF ; ENABLE_GCP
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;FUNCTION to Copy a single line from an unaligned source to an aligned destination
